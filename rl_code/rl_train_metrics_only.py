# rl_train.py — Updated for Rasa 3.6+ (synchronous load + async-safe NLU)
"""
RL fine-tuning for your Rasa TEDPolicy model (Behavior Cloning + REINFORCE).
...
# Clip per-step reward to avoid outliers (helps keep global stats sensible)
REWARD_CLIP_MIN = -5.0
REWARD_CLIP_MAX = 5.0

# New hyperparams for stability/exploration
ENTROPY_COEF = 0.01       # entropy regularization coefficient
MAX_GRAD_NORM = 0.5       # gradient clipping
UPDATE_EVERY = 5          # accumulate gradients over this many episodes before optimizer.step()
BASELINE_WINDOW = 100     # moving-window baseline length (episodes)
SCHED_STEP = 200          # LR scheduler step size
SCHED_GAMMA = 0.7         # LR decay factor
...
REWARD_MAP = {
    ...
    "ask_about_mental_health": 3, "ask_coping": 3, "sad": -1, "stressed": -1,
    "worthless": -2, "anxious": -2, "overwhelmed": -2, "lonely": -2,
    "hopeless": -2, "depressed": -3, "scared": -3, "angry": -3, "worried": -2,
    "hurt": -3, "suicide": -5, "death": -4, "hate-you": -3, "hate-me": -3,
    "stupid": -3, "deny": -1, "done": 1, "not-talking": 1, "sleep": 1,
    "understand": 2, "user-meditation": 2, "meditation": 2,
    "pandora-useful": 1, "time_penalty": -0.1,
}
...
class EnhancedUserSimulator:
    ...
    def respond(self, bot_action_idx, actions_list):
        ...
        # Clip per-step reward to prevent extreme outliers inflating episode sums
...
# ----------------------------
# REINFORCE training (with baseline & batch updates & entropy)
# ----------------------------
def reinforce_train(policy_net, simulator, actions_list, intent_to_idx, num_slots, episodes=500):
    ...
    episode_mean_rewards = []   # average reward per timestep per episode
    episode_total_rewards = []  # raw total reward per episode (sum of per-step rewards)

    # moving-window baseline for advantage estimation
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # terminal bonus for positive terminal states (clipped)
            if done and r > 0:
                r = float(np.clip(r + 5.0, REWARD_CLIP_MIN, REWARD_CLIP_MAX))
        ...
        # Per-episode totals & normalized mean-per-step
        ...
        # Append to baseline window and compute baseline as recent-window mean if available
        baseline_window.append(mean_per_timestep)
        baseline = float(np.mean(baseline_window)) if len(baseline_window) > 0 else 0.0

        # Compute discounted returns (standard)
        G = 0.0
        returns = []
        ...
        # Advantages = returns - baseline (broadcast)
        ...
        # Compute policy loss (REINFORCE with baseline and entropy regularization)
        policy_losses = [-lp * adv for lp, adv in zip(log_probs, advantages)]
        policy_loss = torch.stack(policy_losses).sum()
        ent_loss = -ENTROPY_COEF * torch.stack(entropies).sum()  # maximize entropy -> minimize -entropy
        loss = policy_loss + ent_loss

        # Accumulate gradients for batch update
        loss = loss / UPDATE_EVERY  # scale down so accumulation equals batch average
        loss.backward()

        accumulated_count += 1

        if accumulated_count >= UPDATE_EVERY:
            # gradient clipping
        ...
        if (ep + 1) % 50 == 0:
            ...
            print(f"[REINFORCE] Episode {ep+1}/{episodes}, mean_reward(last50): {recent_mean:.3f}, baseline(window mean): {baseline:.3f}, lr: {current_lr:.2e}")

    # if any gradients left (less than UPDATE_EVERY at end), step once
...
def main():
    ...
    # Re-initialize optimizer for RL (ensures BC step doesn't bias optimizer state)
    print("🔁 Initializing RL optimizer and scheduler...")
    # optimizer & scheduler are created inside reinforce_train, so no need to recreate here

    print("🎯 Starting REINFORCE fine-tuning (improved)...")
    ...
    # ----------------------------
    # Compute and save statistics (on mean-per-timestep rewards)
    # ----------------------------
    ...
    print("\n📈 Reinforcement Learning Summary (per-timestep mean rewards)")
    print("-----------------------------------")
    ...
    # Save stats as CSV for reproducibility
    ...
    stats_df.to_csv("rl_training_stats.csv", index=False)
    print("✅ Saved detailed statistics to rl_training_stats.csv")

    # ----------------------------
    # Plot reward curve (smoothed) + percentile shading
    # ----------------------------
    try:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("reward_curve_detailed.png", dpi=150)
        print("📊 Saved detailed reward curve to reward_curve_detailed.png")
    except Exception as e:
        print("⚠️ Detailed plotting failed:", e)

    # Also save raw per-episode arrays for reproducibility / external analysis
    pd.DataFrame({"episode_mean_reward": rewards_array, "episode_total_reward": total_array}).to_csv("rl_episode_rewards.csv", index_label="episode")
    print("✅ Saved per-episode rewards to rl_episode_rewards.csv")
